...
n_body_faces = 0  # number of body faces
...
for i in range(len(lines)):
    if lines[i].split()[1].upper() == 'BODY':
        n_body_faces = int(lines[i + 1].split()[1])
        for j in range(n_body_faces):
...
boundary_faces = wall_faces + body_faces + inlet_faces + outlet_faces
...
for i in range(n_faces):
    ...
    if c1 == c2:  # boundary faces
        ...
        Ef.append(np.dot(Sf[i]*np.array(nf[i]), Sf[i]*np.array(nf[i])
                         )/np.dot(Sf[i]*np.array(nf[i]), np.array(ef[i])))
        Tf.append([Sf[i]*nf[i][0] - Ef[i]*ef[i][0],
                  Sf[i]*nf[i][1] - Ef[i]*ef[i][1]])
...
if check_mesh:
    ...
    for i in range(n_cells):
        ...
        plt.text(ccx, ccy, i, c='b', horizontalalignment='center',
                 verticalalignment='center')

    for i in range(n_faces):
        fcx = fc[i][0]  # face centre x-coordinate
        fcy = fc[i][1]  # face centre y-coordinate
        plt.text(fcx, fcy, i, c='r', horizontalalignment='center',
                 verticalalignment='center')

    for i in range(n_nodes):
        nx = nodes[i][0]  # node x-coordinate
        ny = nodes[i][1]  # node y-coordinate
        plt.text(nx, ny, i, c='k', horizontalalignment='center',
                 verticalalignment='center')
    ...
    plt.arrow(fc[fi][0], fc[fi][1], 0.1*nf[fi][0], 0.1 *
              nf[fi][1], color='green', head_width=0.02)
    plt.arrow(fc[fi][0], fc[fi][1], 0.1*tf[fi][0], 0.1 *
              tf[fi][1], color='orange', head_width=0.02)
    ...
    for j in range(3):
        fj = cell_faces[ci][j]
        sign = nt[ci][j]
        plt.arrow(fc[fj][0], fc[fj][1], sign*Sf[fj]*nf[fj][0],
                  sign*Sf[fj]*nf[fj][1], color='green', head_width=0.02)
        plt.arrow(fc[fj][0], fc[fj][1], sign*Sf[fj]*tf[fj][0],
                  sign*Sf[fj]*tf[fj][1], color='orange', head_width=0.02)

        plt.arrow(cc[ci][0], cc[ci][1], sign*df[fj]*ef[fj][0],
                  sign*df[fj]*ef[fj][1], color='grey', head_width=0.02)
        plt.arrow(cc[ci][0], cc[ci][1], sign*hf[fj]*nf[fj][0],
                  sign*hf[fj]*nf[fj][1], color='black', head_width=0.02)
        plt.arrow(fc[fj][0], fc[fj][1], sign*Ef[fj]*ef[fj][0],
                  sign*Ef[fj]*ef[fj][1], color='cyan', head_width=0.02)
        plt.arrow(fc[fj][0] + sign*Ef[fj]*ef[fj][0], fc[fj][1] + sign*Ef[fj]*ef[fj]
                  [1], sign*Tf[fj][0], sign*Tf[fj][1], color='magenta', head_width=0.02)
...
for n in range(itr):
    ...
    for fi in interior_faces:
        ...
        dfi = df[fi]
        efi = np.array(ef[fi])
        grad_uf[fi] = grad_uf_bar + \
            ((u[c2] - u[c1])/dfi - np.dot(grad_uf_bar, efi))*efi
        grad_vf[fi] = grad_vf_bar + \
            ((v[c2] - v[c1])/dfi - np.dot(grad_vf_bar, efi))*efi
    for fi in inlet_faces + wall_faces + body_faces:
        c1 = face_cells[fi][0]  # cell C
        dfi = df[fi]
        efi = np.array(ef[fi])
    ...
    for fi in interior_faces:
        ...
        dfi = df[fi]
        efi = np.array(ef[fi])
        grad_pf[fi] = grad_pf_bar + \
            ((p[c2] - p[c1])/dfi - np.dot(grad_pf_bar, efi))*efi
    ...
    for ci in interior_cells:
        ...
        for j in range(3):
            ...
            if u[ci] == u[cj]:
                ...
                bCu[ci] += nu*np.dot(grad_uf[fj], Tfj) - 0.5*(u[cj] - u[ci]) * \
                    (psip*max(mfj, 0) + psim*max(-mfj, 0)) - pf[fj]*Sfj*nfj[0]
            ...
            if v[ci] == v[cj]:
                ...
                psip = max(0, min(2*rfp, (rfp + 1)/2, 2))
                psim = max(0, min(2*rfm, (rfm + 1)/2, 2))
                bCv[ci] += nu*np.dot(grad_vf[fj], Tfj) - 0.5*(v[cj] - v[ci]) * \
                    (psip*max(mfj, 0) + psim*max(-mfj, 0)) - pf[fj]*Sfj*nfj[1]
    for ci in boundary_cells:
        ...
        for j in range(3):
            ...
            if fj in wall_faces + body_faces:
                aCu[ci] += (nu*Sfj/hfj)*(1 - nfj[0]**2)
                # aFu[ci, j] = 0.0
                bCu[ci] += (nu*Sfj/hfj)*(v[ci]*nfj[0]*nfj[1]) - \
                    pf[fj]*Sfj*nfj[0]

                aCv[ci] += (nu*Sfj/hfj)*(1 - nfj[1]**2)
                # aFv[ci, j] = 0.0
                bCv[ci] += (nu*Sfj/hfj)*(u[ci]*nfj[0]*nfj[1]) - \
                    pf[fj]*Sfj*nfj[1]
    ...
    for fi in interior_faces:
        ...
        Sfi = Sf[fi]*np.array(nf[fi])
        Spfi = Dfi*Sfi
        msf[fi] = np.dot(Vsf_bar, Sfi) - \
            np.dot(grad_pf[fi] - grad_pf_bar, Spfi)
    for fi in outlet_faces:
        c1 = face_cells[fi][0]  # cell C
...
for i in range(n_cells):
    points[i, 0] = cc[i][0]
    points[i, 1] = cc[i][1]
grid_x, grid_y = np.mgrid[0:L:complex(0, 1e3*L), 0:H:complex(0, 1e3*H)]
grid_z = griddata(points, values, (grid_x, grid_y), method='linear')
plt.imshow(grid_z.T, extent=(0, L, 0, H), origin='lower', cmap='jet')
plt.colorbar(orientation='horizontal', label='Speed [m/s]', shrink=0.5)

plt.title('Re = %d, U = %.1f m/s, ν = %.3f m²/s' %
          (U*D/nu, U, nu), fontweight='bold')
