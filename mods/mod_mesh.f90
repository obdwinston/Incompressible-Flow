module mod_mesh
    ...
    type, extends(Element) :: Face
        ...
        ! === incompressible solver ===
        real(real64), dimension(2) ::                   face_tangent
        real(real64) ::                                 face_cell_weight
        real(real64), dimension(2) ::                   face_cell_distances ! [df, ef]
        real(real64), dimension(2) ::                   face_cell_vector ! Lf
        ! === incompressible solver ===
    end type Face
...
contains
    ...
    subroutine set_nodes(self, file_unit)
        ...
        do i = 1, self % n_nodes
            ...
            ! === incompressible solver ===!
            self % nodes(i) % node_type = 'INTERIOR'
            ! === incompressible solver ===!
        end do
    ...
    subroutine set_boundary_faces(self, c1, l1, l2, file_unit, k)
        ...
        do i = 1, k
            ...
            do j = 1, self % n_face_types(i)
                ...
                l2 = [l2, n2 + 1]
                ! === incompressible solver ===!
                self % nodes(n1 + 1) % node_type = self % face_types(i) ! wall last boundary
                self % nodes(n2 + 1) % node_type = self % face_types(i) ! wall last boundary
                ! === incompressible solver ===!
            end do
        end do
    ...
    subroutine set_face_components(self)
        ...
        integer(int32) :: c1, c2
        real(real64) :: d1, d2
        real(real64), dimension(2) :: Lf, nf, tf
        ! === incompressible solver ===
        do i = 1, self % n_faces
            ...
            ! === incompressible solver ===
            self % faces(i) % face_tangent = &
            [(n2x - n1x)/self % faces(i) % face_area, &
            (n2y - n1y)/self % faces(i) % face_area]            
            c1 = self % faces(i) % face_cells(1)
            c2 = self % faces(i) % face_cells(2)
            if (c1 == c2) then ! boundary face
                Lf = self % faces(i) % coordinates - &
                self % cells(c1) % coordinates
            else ! interior face
                Lf = self % cells(c2) % coordinates - &
                self % cells(c1) % coordinates
            end if
            self % faces(i) % face_cell_vector = Lf
            self % faces(i) % face_cell_distances(1) = &
            abs(dot_product(Lf, self % faces(i) % face_normal))
            self % faces(i) % face_cell_distances(2) = &
            abs(dot_product(Lf, self % faces(i) % face_tangent))
            d1 = 1/norm2(self % cells(c1) % coordinates - &
            self % faces(i) % coordinates)
            d2 = 1/norm2(self % cells(c2) % coordinates - &
            self % faces(i) % coordinates)
            self % faces(i) % face_cell_weight = d1/(d1 + d2)
            ! === incompressible solver ===
        end do
        ...
        do i = 1, self % n_cells
            ...
            do j = 1, 3
                ...
                if (i == self % faces(fj) % face_cells(1)) then
                    ! === incompressible solver ===
                    self % cells(i) % cell_cells(j) = self % faces(fj) % face_cells(2)
                    ! === incompressible solver ===
                    self % cells(i) % cell_face_signs(j) = 1.0
                else
                    ! === incompressible solver ===
                    self % cells(i) % cell_cells(j) = self % faces(fj) % face_cells(1)
                    ! === incompressible solver ===
                    self % cells(i) % cell_face_signs(j) = -1.0
                end if
            end do
