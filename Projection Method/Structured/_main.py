...
aE[-2, 1:-1] = 0.0  # outlet (Dirichlet zero pressure outlet)
aW[1, 1:-1] = 0.0  # inlet
...
aP[-2, 1:-1] += -(1/dx/dx)  # outlet (Dirichlet zero pressure outlet)
...
for n in range(nt):
    ...
    FucN[2:-1, 1:-2] = vN[2:-1, 1:-2]*.5*(u[2:-1, 1:-2] + u[2:-1, 2:-1])*dx  # centred difference
    # FucN[2:-1, 1:-2] = (np.maximum(vN[2:-1, 1:-2], 0)*u[2:-1, 1:-2] - np.maximum(-vN[2:-1, 1:-2], 0)*u[2:-1, 2:-1])*dx  # upwind difference
    ...
    FucS[2:-1, 2:-1] = vS[2:-1, 2:-1]*.5*(u[2:-1, 1:-2] + u[2:-1, 2:-1])*dx  # centred difference
    # FucS[2:-1, 2:-1] = (np.maximum(vS[2:-1, 2:-1], 0)*u[2:-1, 1:-2] - np.maximum(-vS[2:-1, 2:-1], 0)*u[2:-1, 2:-1])*dx  # upwind difference
    ...
    FucE[2:-2, 1:-1] = uE[2:-2, 1:-1]*.5*(u[2:-2, 1:-1] + u[3:-1, 1:-1])*dy  # centred difference
    # FucE[2:-2, 1:-1] = (np.maximum(uE[2:-2, 1:-1], 0)*u[2:-2, 1:-1] - np.maximum(-uE[2:-2, 1:-1], 0)*u[3:-1, 1:-1])*dy  # upwind difference
    ...
    FucW[3:-1, 1:-1] = uW[3:-1, 1:-1]*.5*(u[2:-2, 1:-1] + u[3:-1, 1:-1])*dy  # centred difference
    # FucW[3:-1, 1:-1] = (np.maximum(uW[3:-1, 1:-1], 0)*u[2:-2, 1:-1] - np.maximum(-uW[3:-1, 1:-1], 0)*u[3:-1, 1:-1])*dy  # upwind difference
    ...
    FvcN[1:-1, -2] = vN[1:-1, -2]*.5*(v[1:-1, -2] + v[1:-1, -1])*dx  # centred difference
    # FvcN[1:-1, -2] = (np.maximum(vN[1:-1, -2], 0)*v[1:-1, -2] - np.maximum(-vN[1:-1, -2], 0)*v[1:-1, -1])*dx  # upwind difference
    # inner cells
    FvdN[1:-1, 2:-2] = nu*(v[1:-1, 3:-1] - v[1:-1, 2:-2])/dy*dx
    FvcN[1:-1, 2:-2] = vN[1:-1, 2:-2]*.5*(v[1:-1, 2:-2] + v[1:-1, 3:-1])*dx  # centred difference
    # FvcN[1:-1, 2:-2] = (np.maximum(vN[1:-1, 2:-2], 0)*v[1:-1, 2:-2] - np.maximum(-vN[1:-1, 2:-2], 0)*v[1:-1, 3:-1])*dx  # upwind difference

    # south flux
    # bottom wall
    FvdS[1:-1, 2] = nu*v[1:-1, 2]/dy*dx
    FvcS[1:-1, 2] = vS[1:-1, 2]*.5*(v[1:-1, 1] + v[1:-1, 2])*dx  # centred difference
    # FvcS[1:-1, 2] = (np.maximum(vS[1:-1, 2], 0)*v[1:-1, 1] - np.maximum(-vS[1:-1, 2], 0)*v[1:-1, 2])*dx  # upwind difference
    # inner cells
    FvdS[1:-1, 3:-1] = nu*(v[1:-1, 3:-1] - v[1:-1, 2:-2])/dy*dx
    FvcS[1:-1, 3:-1] = vS[1:-1, 3:-1]*.5*(v[1:-1, 2:-2] + v[1:-1, 3:-1])*dx  # centred difference
    # FvcS[1:-1, 3:-1] = (np.maximum(vS[1:-1, 3:-1], 0)*v[1:-1, 2:-2] - np.maximum(-vS[1:-1, 3:-1], 0)*v[1:-1, 3:-1])*dx  # upwind difference

    # east flux
    # outlet
    FvdE[-2, 2:-1] = 0.0
    FvcE[-2, 2:-1] = uE[-2, 2:-1]*.5*(v[-2, 2:-1] + v[-1, 2:-1])*dy  # centred difference
    # FvcE[-2, 2:-1] = (np.maximum(uE[-2, 2:-1], 0)*v[-2, 2:-1] - np.maximum(-uE[-2, 2:-1], 0)*v[-1, 2:-1])*dy  # upwind difference
    # inner cells
    FvdE[1:-2, 2:-1] = nu*(v[2:-1, 2:-1] - v[1:-2, 2:-1])/dx*dy
    FvcE[1:-2, 2:-1] = uE[1:-2, 2:-1]*.5*(v[1:-2, 2:-1] + v[2:-1, 2:-1])*dy  # centred difference
    ...
    FvcW[2:-1, 2:-1] = uW[2:-1, 2:-1]*.5*(v[1:-2, 2:-1] + v[2:-1, 2:-1])*dy  # centred difference
    # FvcW[2:-1, 2:-1] = (np.maximum(uW[2:-1, 2:-1], 0)*v[1:-2, 2:-1] - np.maximum(-uW[2:-1, 2:-1], 0)*v[2:-1, 2:-1])*dy  # upwind difference
...
for n in range(0, nt, interval):
    ...
    uavg = .5*(u[1:-1, 1:-1] + u[2:, 1:-1])
    vavg = .5*(v[1:-1, 1:-1] + v[1:-1, 2:])
    w = np.sqrt(uavg*uavg + vavg*vavg)
    # dvdx = (v[2:, 2:] + v[2:, 1:-1] - v[:-2, 2:] - v[:-2, 1:-1])/(4*dx)
    # dudy = (u[1:-1, 2:] + u[2:, 2:] - u[1:-1, 1:-1] - u[2:, 1:-1])/(4*dy)
    # w = dvdx - dudy

    cmap = plt.get_cmap('jet')
    levels = np.linspace(0, limit, int(10*limit) + 1)
    ...
    # levels = np.linspace(-limit, limit, int(10*limit) + 1)
    # plt.contourf(xx, yy, np.transpose(w), cmap=cmap, levels=levels, extend='both')
    # plt.colorbar(orientation='horizontal', label='Vorticity [/s]', shrink=0.5)
...
image = []
for n in range(0, nt, interval):
    image.append(figure + '/figure{:d}.png'.format(n))
clip = mpy.ImageSequenceClip(image, fps=int(len(image)/t/factor))
clip.write_videofile(figure + '/animation.mp4')
